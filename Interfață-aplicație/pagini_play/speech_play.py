...
def play_activitate_speech():
    label_speech.pack_forget()
    button_play_speech.pack_forget()
    frame_button_back_nivel.pack_forget()
    label_final.pack(pady=10)
    button_final.pack(pady=10)
    global nume_activitate, data_start
    nume_activitate = "Recunoaștere animale pe baza idescrierilor - n3"
    data_start = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    os.system('C:\\Python27\\python.exe C:\\.Partitie_cu_informatii\\An4\\Licenta\\play_activities\\speechbh.py')
...
def fereastra_speech(root, frame_nivel, frame_button_back, frame_deconectare, cnp_copil, tabel_date):
    global frame_pachet, label_speech, button_play_speech
    frame_pachet = tk.Frame(root, bg="white")
    frame_pachet.pack(fill=tk.BOTH, padx=20, pady=180)

    label_speech = tk.Label(frame_pachet, text="Pachetul pentru activitatea de recunoaștere\na animalelor pe baza descrierilor s-a instalat!\nSă începem activitatea!",
                             font=("Verdana", 18), background="white",
                             width=40, height=3, justify="center")
    label_speech.pack(pady=10)

    button_play_speech = tk.Button(frame_pachet, text="Play activitate\nSpeech Recognition",
                                    background="white",
                                    font=("Verdana", 12),
                                    width=20, height=3, command=lambda:play_activitate_speech(cnp_copil, tabel_date))
    button_play_speech.pack(pady=10)
