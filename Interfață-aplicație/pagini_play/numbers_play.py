...
def play_activitate_numbers():
    label_numbers.pack_forget()
    button_play_numbers.pack_forget()
    frame_button_back_nivel.pack_forget()
    label_final.pack(pady=10)
    button_final.pack(pady=10)
    global nume_activitate, data_start
    nume_activitate = "Învățare cifre - n1"
    data_start = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    os.system('C:\\Python27\\python.exe C:\\.Partitie_cu_informatii\\An4\\Licenta\\play_activities\\numberbh.py')
...
def fereastra_numbers(root, frame_nivel, frame_button_back, frame_deconectare, cnp_copil, tabel_date):
    global frame_pachet, label_numbers, button_play_numbers
    frame_pachet = tk.Frame(root, bg="white")
    frame_pachet.pack(fill=tk.BOTH, padx=20, pady=180)

    label_numbers = tk.Label(frame_pachet, text="Pachetul pentru activitatea de învățare\na cifrelor s-a instalat!\nSă începem activitatea!",
                                font=("Verdana", 18), background="white",
                                width=40, height=3, justify="center")
    label_numbers.pack(pady=10)

    button_play_numbers = tk.Button(frame_pachet, text="Play activitate\nLearning numbers", background="white",
                                  font=("Verdana", 12),
                                  width=15, height=3, command=lambda :play_activitate_numbers(cnp_copil, tabel_date))
    button_play_numbers.pack(pady=10)
